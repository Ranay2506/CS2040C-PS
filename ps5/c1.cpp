...
int Query(int s, int t, int k) {
  ...
  //
  // write your answer here
  nodedistance[s].fill(1000000); // Set distance to be -1 for every node
  queue<ii> qu;
  int vertex;
  int depth[1000];
  ...
  while(!qu.empty()) {
    ...
    for(auto iter: AdjList[vertex]) {
      ...
      //cout << nodedistance[s][vertex] + weight << endl;
      if(nodedistance[s][v] > nodedistance[s][vertex] + weight) {
        cout << "Worked";
        nodedistance[s][v] = nodedistance[s][vertex] + weight;
        qu.push(make_pair(nodedistance[s][v],v));
        depth[v] += 1;
      }
    }
  }

  if (nodedistance[s][t] == 1000000) {
    return -1;
  } else {
